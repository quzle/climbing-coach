...
sequenceDiagram
    ...
    Supabase-->>User: Invite email with magic link

    Note over User,Supabase: Account activation
    User->>App: Clicks magic link → /auth/callback?code=...
...
|---|---|---|---|
| **Receive invite** | Opens email from Supabase | Generic Supabase-branded invite email with a "Accept invite" button | Email is sent from Supabase's default address with Supabase branding. There is no app-specific copy, no mention of what Climbing Coach is, and no indication of who sent the invite. |
| **Click magic link** | Clicks "Accept invite" | Browser opens `/auth/callback?code=...` | User has no visibility of what is happening. No loading state is shown during the callback. |
| **Session established** | — (automatic) | `exchangeCodeForSession` runs; cookies are set; profile row is upserted | If the code exchange fails the user is redirected to `/auth/login?error=callback_failed` with no explanation of what went wrong or what to do next. |
...
- **Silent callback failures.** If `exchangeCodeForSession` or profile finalization fails, the user is redirected to `/auth/login?error=callback_failed` with no explanation in the UI about what went wrong or what to do next.
